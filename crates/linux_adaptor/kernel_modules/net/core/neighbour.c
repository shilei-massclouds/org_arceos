...
void neigh_table_init(int index, struct neigh_table *tbl)
{
	...
	pr_warn("neigh_table_init: enter idx=%d id=%s\n", index, tbl->id);
	...
	pr_warn("neigh_table_init: alloc_percpu stats\n");
	tbl->stats = alloc_percpu(struct neigh_statistics);
	if (!tbl->stats)
		panic("cannot create neighbour cache statistics");

#ifdef CONFIG_PROC_FS
	pr_warn("neigh_table_init: proc_create_seq_data\n");
	if (!proc_create_seq_data(tbl->id, 0, init_net.proc_net_stat,
			      &neigh_stat_seq_ops, tbl))
		panic("cannot create neighbour proc dir entry");
#endif

	pr_warn("neigh_table_init: neigh_hash_alloc\n");
	RCU_INIT_POINTER(tbl->nht, neigh_hash_alloc(3));

	phsize = (PNEIGH_HASHMASK + 1) * sizeof(struct pneigh_entry *);
	pr_warn("neigh_table_init: kzalloc phash\n");
	...
	pr_warn("neigh_table_init: rwlock_init\n");
	rwlock_init(&tbl->lock);

	pr_warn("neigh_table_init: schedule gc/mananged work\n");
	...
	pr_warn("neigh_table_init: timer/skb init\n");
	...
	pr_warn("neigh_table_init: publish table\n");
	rcu_assign_pointer(neigh_tables[index], tbl);
	pr_warn("neigh_table_init: done\n");
}
...
{
	pr_warn("neigh_sysctl_register: skipped\n");
	return 0;
#if 0
	int i;
