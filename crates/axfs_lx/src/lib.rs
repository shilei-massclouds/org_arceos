...
fn init_filesystems(mut blk_devs: AxDeviceContainer<AxBlockDevice>) {
...
axstage::register!("AxFS", AxStage::InitFS, |_, _| {
    let all_devices = axdriver::get_all_devs();
    if let Some(block) = all_devices.lock().block.take() {
        init_filesystems(block);
    } else {
        panic!("No block devices.");
    }
});
