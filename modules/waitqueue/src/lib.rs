...
pub type WaiterRef = Arc<dyn Waiter>;
...
///
/// ```
/// use axtask::AxWaitQueue;
/// use core::sync::atomic::{AtomicU32, Ordering};
///
/// static VALUE: AtomicU32 = AtomicU32::new(0);
/// static WQ: AxWaitQueue = AxWaitQueue::new();
///
...
/// ```
pub struct AxWaitQueue {
    queue: SpinRaw<VecDeque<WaiterRef>>, // we already disabled IRQs when lock the `RUN_QUEUE`
}
...
/*
    ...
    fn cancel_events(&self, curr: CurrentTask) {
        ...
        if curr.in_timer_list() {
            // timeout was set but not triggered (wake up by `AxWaitQueue::notify()`)
    ...
    */
    ...
    pub fn wait_until<F>(&self, condition: F, current: WaiterRef)
    where
        F: Fn() -> bool,
    {
        loop {
