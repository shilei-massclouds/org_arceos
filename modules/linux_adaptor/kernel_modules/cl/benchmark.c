...
static void testcase_read(const char *path, int count)
{
    uint64_t t0, t1;
    t0 = get_ticks();
    ...
    for (int i = 0; i < 1000; i++) {
        test_file_read(path, rbuf, count, 0);
    }

    t1 = get_ticks();
    printk("READ [%d]: ticks: %lu\n", count, t1 - t0);
}

static void testcase_write(const char *path, int count)
{
    uint64_t t0, t1;
    t0 = get_ticks();
    ...
    char wbuf[4096];
    for (int i = 0; i < 1000; i++) {
        test_file_write(path, wbuf, count, 0);
    }

    t1 = get_ticks();
    printk("WRITE [%d]: ticks: %lu\n", count, t1 - t0);
}
...
static void benchmark(void)
{
    ...
    test_file_write(path, wbuf, sizeof(wbuf), 0);

    testcase_read(path, 64);
    testcase_read(path, 512);
    testcase_read(path, 1024);
    testcase_read(path, 4096);

    testcase_write(path, 64);
    testcase_write(path, 512);
    testcase_write(path, 1024);
    testcase_write(path, 4096);
