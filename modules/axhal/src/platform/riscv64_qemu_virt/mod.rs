...
unsafe extern "C" {
    fn riscv_hartid_to_cpuid(hartid: usize) -> isize;
    fn idle_thread_get(cpuid: usize) -> usize;
...
#[cfg(feature = "smp")]
unsafe extern "C" fn rust_entry_secondary(hartid: usize) {
    let cpu_id = riscv_hartid_to_cpuid(hartid);
    assert!(cpu_id > 0);

    // set tp for clinux kthread.
    let tp = idle_thread_get(cpu_id as usize);
    crate::arch::write_thread_pointer(tp);

    crate::cpu::init_secondary(cpu_id as usize);
    rust_main_secondary(cpu_id as usize);
}
